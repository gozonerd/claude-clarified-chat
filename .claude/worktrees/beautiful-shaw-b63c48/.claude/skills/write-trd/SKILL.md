...
- When the user invokes `/write-trd` or an equivalent trigger
...
### Step 1: Verify PRD Prerequisite

Before loading the template, verify a completed PRD exists and is approved. If missing:
- If the user has said "skip the PRD" or "the PRD will come later" — refuse to proceed. A TRD written without a PRD is ungrounded. Offer to invoke `/write-prd` first.
